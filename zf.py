...
if step == 'getPhoneForLogin' and text.replace('+', '').replace(' ', '').replace('-', '').isdigit():
    ...
    try:
        ...
        await message.reply(
            "<b>کد وارد شده نامعتبر است یا منقضی شده است ❌</b>"
        )
...
• زمان سپری شده : {spendTime}''')
    ...
    # ----------------- Group report (PersonalDetails) -----------------
         ...
         try:
             ...
             return
         report_text = "\n".join(parts[2:])
         links = [l.strip() for l in tempReportLink_personal.splitlines() if l.strip()]
         total_reports = len(links) * acc_count * rpt_count
         accounts = await accountList()
         chosen_accounts = accounts[:acc_count]
         all_reports = 0
...
• زمان سپری شده : {spendTime}''')
    ...
    # ----------------- Group report (Copyright) -----------------
         ...
         try:
             ...
             return
         report_text = "\n".join(parts[2:])
         links = [l.strip() for l in tempReportLink_copyright.splitlines() if l.strip()]
         total_reports = len(links) * acc_count * rpt_count
         accounts = await accountList()
         chosen_accounts = accounts[:acc_count]
         all_reports = 0
...
async def main():
    ...
    await idle()  # Wait until the bot is stopped manually
    await bot.stop()
